...
fun prove_simple_action opr a cm postfix =
    let 
	
	val thrm = (DB.fetch  (current_theory()) ((Hol_pp.term_to_string (opr)) ^ postfix))

	val _ =  proof_progress ("\nTheorem " ^ (term_to_string opr) ^ postfix ^"thm is applied!\n ")

	val tacs = ASSUME_TAC vector_table_adr_thm 
		      THEN RW_TAC (srw_ss()) [SPEC cm thrm] 
		      THEN (ASSUME_TAC (SPEC cm thrm) THEN
...
fun prove_const  a pred expr cm postfix thms =
	...
	val prove_composite_action = 
	 fn (l, r, opr, a,pred,expr, cm,postfix,thms) =>
		...
		val (part , comb_thm) =
			...
			in
			    if (same_const oprr ``return``)
			    then
				(l , SPECL [l,``ii:iiid``] 
					    return_preserves_pc_thm)
			    else
				(r , SPECL [ l, r,cm]
			...
			(r , SPECL [ l, r,cm]
    			       (INST_TYPE [beta |-> get_monad_type(snd),
    					   alpha  |-> l_type]
    					  (List.nth(thms, 2))))
		...
		val analyze_action =
		 fn (a, l , r, opr,pred,expr,cm,postfix,thms) =>
			...
			let val _ = proof_progress ("\n\nProof of a simple action:\n "^(term_to_string l)^"\n\n")
	  		    ...
	  		    (* if (thm_exist) *)
	  		    (* then *)
	  		    prove_simple_action l a cm postfix 
	  		(* else *)
		...
		val prove_abs_action =
		 fn (a, pred ,expr ,cm ,postfix ,thms) =>
			...
			val proved_unbeta_lemma = store_thm ("proved_unbeta_lemma",
							     ``(set_pc_to ^a_body ^cm)=
						  (set_pc_to ^unbeta_a  ^cm)``,
							     (ASSUME_TAC (SPECL [a_body,``^unbeta_a``, cm]
	...
	val const_prove_abs_action =
	 fn (a, proved_a,thms) =>
	    let
		val (a_abs,a_body) = pairLib.dest_pabs a;
		val _ =  set_goal([], ``
				 (priv_flags_constraints_abs ii ^a ) ``)
		val _ = e (FULL_SIMP_TAC (let_ss) [])  (*change of Oliver*)
		...
		val proved_unbeta_lemma = store_thm ("proved_unbeta_lemma",
						     ``(priv_flags_constraints ii ^a_body)=
					  (priv_flags_constraints ii ^unbeta_a)``,
						     (ASSUME_TAC (SPECL [a_body,``^unbeta_a``, ``ii:iiid``]
									(INST_TYPE [beta |-> a_body_type,
										    alpha |-> ``:arm_state``] 
										   (List.nth(thms,3)))))
							 THEN ASSUME_TAC unbeta_thm
