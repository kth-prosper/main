...
fun build_right_hand_side (src_inv, trg_inv,
	...
	(* then  *)
	(*     let  *)
	(* 	val a_thm = SPECL [src_inv, rtrg_inv, trg_inv, ltrg_inv, r]  *)
...
fun prove a src_inv trg_inv uargs pthms  =
	...
	val prove_composite_action = 
	 fn (l, r, opr, a, src_inv, trg_inv,uargs,pthms) =>
		...
		val is_write_cpsr = (same_const  oprr ``write_cpsr``)
		...
		val tacsb =
	  	    if (is_write_cpsr)
		    then
			FULL_SIMP_TAC (srw_ss()) [List.nth(pthms,4)]
		    else
			(if (same_const  opr ``$>>=``)
	  		 then	  	 	
    ...
    in
	let val (proved_thm,tacs) = case dest_term a of
				 (LAMB (c,b))  =>
				 ...
				 then
					 ...
					 val _ = e (tac)
					 val tac = FULL_SIMP_TAC (srw_ss()) (List.drop(pthms,4)@(!simp_thms_list))
