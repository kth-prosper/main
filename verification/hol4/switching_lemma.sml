...
(* val user_pr_taut_imp_priv_pr2_thm =  *)
(*     store_thm ("user_pr_taut_imp_priv_pr2_thm", *)
...
val arch_version_thm =
    save_thm("arch_version_thm",
	     new_axiom("arch_version_thmX", ``! m n k . preserve_relation_mmu (arch_version <|proc:=0|>)
		      (assert_mode n) (assert_mode  n) ^uf1 ^uy1``));

val read_cpsr_thm =
    save_thm("read_cpsr_thm",
	     new_axiom("read_cpsr_thmX",
		       ``!u. preserve_relation_mmu (read_cpsr <|proc := 0|>) (assert_mode u) (assert_mode u) ^uf1 ^uy1``));
...
(* ================================================+++++========================== *)
(*           proof of preserve relation of write_reg in no access violation case   *)
(* =====================================================+++++===================== *)


(* val write_reg_hlp1_thm = *)
(*     store_thm ("write_reg_hlp1_thm", *)
(* 	       ``preserve_relation_mmu (read_cpsr <|proc := 0|> >>= *)
(* 		 (λcpsr. write_reg_mode <|proc := 0|> (14w,cpsr.M) value))  *)
(* 	      (assert_mode 27w) (assert_mode 27w) ^uf1 ^uy1 = *)
(*     preserve_relation_mmu (read_cpsr <|proc := 0|> >>= *)
(* 	(λcpsr. write_reg_mode <|proc := 0|> (14w,27w) value))  *)
(* 			  (assert_mode 27w) (assert_mode 27w) ^uf1 ^uy1``, *)
(* 	       ASSUME_TAC (SIMP_RULE (bool_ss) [] *)
(* 				     (SPECL [``(λcpsr:ARMpsr. write_reg_mode <|proc := 0|> (14w,cpsr.M) value)``, *)
	...
	(?e. (f2  s1 = Error e) /\  (f2  s2 = Error e) ))
	  ``,
	       FULL_SIMP_TAC (let_ss) [preserve_relation_mmu_abs_def,
				       ...
				       ARM_READ_CPSR_def,write_scr_cpsr_preconds_def]
			     THEN RW_TAC (srw_ss()) []
			     THEN Cases_on `(return () ||| write_cpsr <|proc := 0|> (s2.psrs (0,CPSR) with M := u)) s1`
			     THEN  Cases_on `(return () ||| write_cpsr <|proc := 0|> (s2.psrs (0,CPSR) with M := u)) s2`
			     (*THEN FULL_SIMP_TAC (srw_ss())  []*)
			     THEN FIRST_PROVE
			     [ TAKE_EXCEPTION_MODE_CHANGING_ERROR_TAC ``u:bool[5]``,
			       RW_TAC (srw_ss()) [seqT_def]
...
fun prove_take_exception_priv_constraints_thm
  ...
  in
      RW_TAC (bool_ss) [priv_mode_constraints_v3_def,priv_mode_constraints_v2_def,
			priv_mode_constraints_v1_def,satisfy_priv_constraints_v2_def,
			satisfy_priv_constraints_v3_def]
	     THEN FIRST_PROVE
	     [
...
val take_svc_exception1_thm = 
store_thm ("take_svc_exception1_thm",
let
    val athm = SIMP_CONV (bool_ss) [take_svc_exception_def]
			 ``take_svc_exception <|proc:=0|> ``
    val (_, a) =  (dest_eq (concl athm))
    val (_,_,rb)= decompose_term a
in
``preserve_relation_mmu  ^rb  (assert_mode 16w ) 
	      (comb_mode 16w 19w )
	      priv_mode_constraints_v2 priv_mode_similar``
end
,
ASSUME_TAC  take_svc_exception_av_thm
...
val IT_advance_thm =
    save_thm("IT_advance_thm",
	     new_axiom("IT_advance_thmX", ``preserve_relation_mmu (IT_advance <|proc:=0|>)
		      (assert_mode 16w) (assert_mode  16w) priv_mode_constraints_v3 priv_mode_similar``));


val uargs = [``priv_mode_constraints_v3``, ``priv_mode_similar``, ``19w``,
      ``_thm``]
val a = ``take_svc_exception <|proc := 0|> ``;
val () = global_mode := ``16w:bool[5]``;
val pthms = pthms @ [take_svc_exception_thm];
val (thm,_) = prove a ``(assert_mode 16w)`` 
		      ``(comb_mode 16w 19w)`` 
		      uargs pthms;
