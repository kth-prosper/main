...
val IT_advance_thm =
    save_thm("IT_advance_thm",
	     new_axiom("IT_advance_thmX", ``preserve_relation_mmu (IT_advance <|proc:=0|>)
		      (assert_mode 16w) (assert_mode  16w) priv_mode_constraints_v2 priv_mode_similar``))


val take_svc_exception_thm = 
    save_thm ("take_svc_exception_thm",
	       let
		   val uargs = [``priv_mode_constraints_v2``, 
				``priv_mode_similar``, ``19w:bool[5]``,
				``_thm``]
		   val a = ``IT_advance  <|proc := 0|> >>=
						    (λu. take_svc_exception_part2 <|proc := 0|> )``;
		   val () = global_mode := ``16w:bool[5]``
		   val src_inv =``(assert_mode 16w)`` 
		   val trg_inv = ``(comb_mode 16w 19w)``
		   val pthms = pthms @ [take_svc_exception_part2_thm];
