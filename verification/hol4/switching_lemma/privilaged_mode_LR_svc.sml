...
(* g ` satisfy_LR_constraints f 19w 2w ==> *)
(* satisfy_LR_constraints (IT_advance <|proc:=0|> >>= (\u. f)) 19w 2w `; *)

(* RW_TAC (srw_ss()) [satisfy_LR_constraints_def] *)
(* THEN Cases_on `IT_advance <|proc := 0|> s` *)
(* THEN IMP_RES_TAC seqT_access_violation_thm *)
(* THEN FULL_SIMP_TAC (srw_ss()) [seqT_def] *)
(* THEN IMP_RES_TAC IT_advance_untouch_security_ex_thm *)
(* THEN RES_TAC *)
(* THEN UNDISCH_TAC ``(IT_advance <|proc := 0|> >>= (λu. f)) s = ValueState a s'`` *)
(* THEN UNDISCH_TAC ``IT_advance <|proc := 0|> s = ValueState a' b`` *)
(* THEN EVAL_TAC *)
(* THEN RW_TAC (srw_ss()) [] *)
(* THEN RES_TAC *)
(* THEN PAT_ASSUM ``∀instr_set.X`` (fn thm => *)
(* 				    ASSUME_SPECL_TAC [``instr_set:InstrSet``] thm ) *)
