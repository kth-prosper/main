...
(*TODO: improve the code of proving of already proven theorems*)

exception not_matched_pattern; 

val true_thm = Q.prove(`T`, FULL_SIMP_TAC (srw_ss()) []);

fun proof_progress s = TextIO.print s; 

val debuglist = ref [("debug start",[``T``])];

fun print_term_list l =
    case l of
         x::xl => let val _ = proof_progress ("\n" ^ (term_to_string x)) in
                  print_term_list xl
                  end
         | _ => proof_progress ("\n\n");

fun debug_out s l =  debuglist  := ((s,l)::(!debuglist));


val doDebug = false;

fun debug_out_old s l =
    if (doDebug) then
...
val mode_changing_comp_list = ref [``take_undef_instr_exception``, ``take_svc_exception``];
val simplist = ref [true_thm];
...
(*
fun get_mode_from_action a = 
    let val _ = debug_out "get mode from action" [a]
	val (c1,d1) = strip_comb a
...
fun prove_simple_action opr a cm uargs =
    let 
        val _ = debug_out "Prove simple action" [opr, a, cm, List.nth(uargs,0), List.nth(uargs,1)]
...
fun decompose a src_inv trg_inv =
    let val _ = debug_out "decompose" [ a, src_inv, trg_inv]
        val (opr, acs) =    
	    case dest_term a of
		(LAMB (b,c)) => 
	     	strip_comb c
...
fun get_uncurried_theorem thm l = 
    let val _ = debug_out "get uncurried thm" []
        val (asm,con) = dest_thm thm
    in
	if( l = 1)
	then
	    thm
...
fun build_right_hand_side (r_type, l_type, src_inv, rtrg_inv, ltrg_inv,
			   trg_inv, rsrc_inv, l, r, a, right_hand_thm,mode_changed,cm,flg,uargs) =
    let val _ = debug_out "build rhs" []
	val  (preserve_relation_thm,preserve_relation_v2_thm) =
	     if (flg)
	     then
	...
	val same_left_mode =  (term_eq src_inv (*OLIVER: lsrc_inv*) ltrg_inv);
	...
	then 
		...
		ASSUME_TAC b_thm 
			   ...
			   THEN RES_TAC
			   THEN ASSUME_TAC (SPECL [l,r, ``16w:bool[5]``,cm,trg_inv]
		...
		let 
		    val operator_tac = 
			ASSUME_TAC (SPECL [l,r, ``16w:bool[5]``,uf_fun,uy_fun]
			     		  (INST_TYPE [alpha |-> l_type,
			     			      beta  |-> r_type']
			     			     base_thm))
		in
		    ASSUME_TAC right_hand_thm
			       THEN operator_tac
			       THEN ASSUME_TAC (get_trans_thm uf_fun) (*changed by Oliver *) 
			       THEN ASSUME_TAC (SPEC rtrg_inv comb_monot_thm)
			       THEN RES_TAC
		end
	    else
		(*************************************************************************)
		(*for computations in user mode AFTER seeing take_exception or write_cpsr*)
		(*************************************************************************)
		ASSUME_TAC right_hand_thm THEN 
			   (if (same_right_mode)
			    then
				ASSUME_TAC  (get_trans_thm uf_fun) (*changed by Oliver *)
					   THEN IMP_RES_TAC (SPECL [l,r,``16w:bool[5]``,uf_fun,uy_fun]
								   (INST_TYPE [alpha |-> l_type,
    									       beta  |-> r_type]		   
									      (if (flg) 
									       then 
					   ...
					   THEN ASSUME_TAC (SPECL [``16w:bool[5]``, cm] comb_mode_thm)
					   THEN ASSUME_TAC (SPECL [src_inv,``(assert_mode ^cm)``, trg_inv,src_inv,a ,uf_fun,uy_fun]
								  (INST_TYPE [alpha |-> get_monad_type(type_of(a))]
									     preserve_relation_comb_v2_thm)) 
			    else
				
				(* (ASSUME_TAC (SPECL [src_inv,``(assert_mode ^cm)``, trg_inv] combv2_thm)) *)
				(*     THEN *) ASSUME_TAC  (get_trans_thm uf_fun) (*changed by Oliver *)
			     	    THEN ASSUME_TAC (SPECL [l,r, ``16w:bool[5]``,cm,trg_inv ,uf_fun,uy_fun]
    							   (INST_TYPE [alpha |-> l_type,
    								       beta  |-> r_type]
    								      base_thm))
				    THEN ASSUME_TAC (SPECL[``16w:bool[5]``,``^cm``] comb_mode_thm)   ) 	
...
fun prove a src_inv trg_inv cm uargs  =
    let 
        val _ = debug_out "Prove" [a, src_inv, trg_inv, cm, List.nth(uargs,0), List.nth(uargs,1)]
	
	val prove_COND_action = 
	 fn (if_part,else_part,condition,a,src_inv,trg_inv,cm,uargs) =>
	    let val _ = proof_progress ("A Conditional action\n\n\n")
			
		val (if_part_thm,tc) = prove if_part src_inv trg_inv cm uargs;
		val (else_part_thm,tc') = prove else_part src_inv trg_inv cm uargs;
	...
	val prove_composite_action = 
	 fn (l, r, opr, a, src_inv, trg_inv, cm, uargs) =>
	    let val _ = proof_progress ("\n\nProof of the compositional action:\n"^(term_to_string a)^"\n\n")
		val (f,ll ,lr , oprr) = decompose r src_inv trg_inv;
		val mode_changed = (same_const  oprr ``write_cpsr``)
				   andalso (not (term_eq ``write_cpsr <|proc := 0|> (cpsr with IT := 0w)`` r)) andalso (not ((get_mode_from_action r) = ``16w:word5``)); (*changed by Oliver *)
		...
		val (lsrc_inv,ltrg_inv) = 
			...
			then
			    ...
			    ( ``assert_mode ^(!global_mode)`` ,  
			      ``assert_mode ^(!global_mode)``);
		val (left_hand_thm , tc) =   prove l  lsrc_inv ltrg_inv cm uargs
		val tacs =  (ASSUME_TAC left_hand_thm) 
		val _ = e (tacs)
                val _ = debug_out "some left part done" [l]
		...
		val (rsrc_inv , rtrg_inv ) = 
			...
			then
				...
				( ``assert_mode ^(!global_mode)`` ,  
				  ``assert_mode ^(!global_mode)``);
                val _ = debug_out "before proving right part" [r];		   
		val (right_hand_thm, tc) = prove r  rsrc_inv rtrg_inv cm uargs;
		val _ = debug_out "some right part done" [r];    
		...
		val tacsb =
			...
			in
			    build_right_hand_side (r_type, l_type, src_inv, rtrg_inv,  ltrg_inv,
						   trg_inv, rsrc_inv, l , r, a, right_hand_thm, mode_changed,cm,true,uargs)
						  THEN RES_TAC
						  THEN (RW_TAC (srw_ss()) [])
			end
		    else
	...
	val prove_constT_action = 
	 fn (a,src_inv, trg_inv,uargs) =>
	    let val _ = debug_out "Prove constT action" [a, src_inv, trg_inv, List.nth(uargs,0), List.nth(uargs,1)]
		val (opr, arg) = 
		    case dest_term a of
			COMB (opr, arg) => (opr, arg)
		      |_ => Raise not_matched_pattern 
		val tac = ASSUME_TAC (get_reflex_thm (List.nth(uargs,0))) (*changed by Oliver*)  
				     THEN RW_TAC (srw_ss()) 
				     [(SPECL [src_inv,arg, 
	...
	val prove_forT_action = 
	 fn (a,src_inv,trg_inv,cm,uargs) =>
		...
		val (sub_thm , aub_tac) = prove action src_inv trg_inv cm uargs
		val snd = get_type_inst (type_of(action), false)
		(*val (str , [fst , snd]) = dest_type (type_of(action))*)
		val r_type = get_monad_type(snd)
		val tacs = ASSUME_TAC sub_thm (*THEN ASSUME_TAC (SPEC action (INST_TYPE [alpha |-> r_type]  forT_preserving_thm)) THEN RW_TAC (srw_ss()) []
		val _ = e (tacs)*)
               	val proved_thm = top_thm()
	  	val _ = proofManagerLib.drop()
	    in
	...
	val prove_condT_action = 
	 fn (a,src_inv,trg_inv,cm,uargs) =>
	    let val (opr, acs) = strip_comb a
	  	val (arg_thm,tc) = prove (List.nth(acs,1)) src_inv trg_inv cm uargs
	  	val tacs =   ASSUME_TAC arg_thm 
					THEN  ASSUME_TAC (get_reflex_thm (List.nth(uargs,0))) (*changed by Oliver*)  
	...
	val prove_case_body =
	 fn (opt,body,flag,src_inv,trg_inv,cm,uargs) =>
	    let val (body_thm,body_tac) = prove body src_inv trg_inv cm uargs
	...
	val prove_case_action =
	 fn (a ,src_inv,trg_inv,cm,uargs) =>
		...
		then
			...
			val proved_cases = 
			    map (fn (opt, body) => prove_case_body (opt, body,true,src_inv,trg_inv,cm,uargs)) cases 
			val (opt , body) = hd(List.rev(case_options))
			val (body_thm,body_tac) = prove body src_inv trg_inv cm uargs
		    ...
		    let  val proved_cases = map (fn (opt, body) => prove_case_body (opt, body,false,src_inv, trg_inv,cm,uargs)) case_options
			 val (proved_thm) = top_thm()
			 val _ = proofManagerLib.drop()			 
		    in
			(proved_thm, NO_TAC) 
	...
	val analyze_action =
	 fn (a, l , r, opr,src_inv,trg_inv,cm,uargs) =>
	    if ((same_const  opr ``$>>=``) orelse
		(same_const  opr ``$|||``))
	    then
		prove_composite_action (l, r, opr ,a,src_inv,trg_inv,cm,uargs)
	    else
		if (same_const  opr ``COND``) then
		    ...
		    in
	  		prove_COND_action (if_part, else_part,List.nth(acs,0),a,src_inv,trg_inv,cm,uargs)
	  	    end
		else
	  	    if (same_const  opr ``constT``) then	    
			prove_constT_action (a,src_inv,trg_inv,uargs) 
		    else  if (same_const  opr ``forT``) then
          		prove_forT_action (a,src_inv,trg_inv,cm,uargs)
          	    else 
			if (TypeBase.is_case a)	then 
			    prove_case_action (a,src_inv,trg_inv,cm,uargs)
			else
			    let val _ = proof_progress ("\n\nProof of a simple action:\n "^(term_to_string l)^"\n\n")
					
  	  			val thm_exist = ((exists_theorem (current_theory()) (term_to_string l ^ "_thm")) orelse (exists_theorem (current_theory()) (term_to_string l ^ "_comb_thm"))) in
	  			if (thm_exist)
	  			then
	  			    prove_simple_action l a cm uargs
	  			else
	  			    prove_condT_action (a,src_inv,trg_inv,cm,uargs)
	  		    end
	val prove_abs_action =
	 fn (a,src_inv,trg_inv,cm,uargs) =>
	    let val _ = debug_out "Prove abstract action" [a, src_inv, trg_inv, cm, List.nth(uargs,0), List.nth(uargs,1)] 
		val uf_fun = List.nth(uargs,0)
		val uy_fun = List.nth(uargs,1)
		val (a_abs,a_body) = pairLib.dest_pabs a;
		val _ =  set_goal([], ``
				 (preserve_relation_mmu_abs (^a) ^src_inv ^trg_inv ^uf_fun ^uy_fun) ``)
		val _ = e (FULL_SIMP_TAC (let_ss) (!simplist))  (*change of Oliver*)
		val (proved_a,b) = prove a_body src_inv trg_inv cm uargs
	 ...
	 fn (a, opr,cm ,uargs) =>
		...
		then
		    let val tac = RW_TAC (srw_ss()) 
					 [preserve_relation_mmu_def,similar_def,
					  readT_def,equal_user_register_def,
					  priv_mode_similar_def,priv_mode_constraints_de,
					  empty_unt_def, (*changed by Oliver *)
					  comb_def,comb_mode_def,assert_mode_def] 
					 THEN FULL_SIMP_TAC (srw_ss()) [untouched_def, readT_def]
					 THEN PAT_ASSUM ``∀ reg.X`` (fn thm => ASSUME_TAC (thm))
			...
			val (proved_a, tacb) =
			    if (flag)
	  		    then
				analyze_action (a', l, r, opr,src_inv,trg_inv,cm,uargs)
			    else
	  			let 
				    val (next_proof , next_tac) = prove a' src_inv trg_inv cm uargs
	...
	case dest_term a of
	    (LAMB (c,b))  =>
	    prove_abs_action (a,src_inv,trg_inv,cm,uargs)
	  | COMB (c,b) =>
		...
		let val uf_fun = List.nth(uargs,0)
		    ...
		    val _ = e (tac)
		    val tac = FULL_SIMP_TAC (srw_ss()) (!simplist) (*changed by Oliver *)
		...
		in
			...
			in
				...
				then
				    ...
				    prove_simple_unproved_action (a', l,cm,uargs)
			end
		end
	  | _ => (ASSUME ``T``, NO_TAC)
    end;
